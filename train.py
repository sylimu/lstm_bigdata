...
tf.flags.DEFINE_integer('n_samples', 1400,#None,
...
tf.flags.DEFINE_integer('batch_size', 500,
                        'Batch size')
tf.flags.DEFINE_integer('train_steps', 120,
                        'Number of training steps')
tf.flags.DEFINE_integer('hidden_size', 100,#75,
...
tf.flags.DEFINE_float('test_size', 0.1,
...
for i in range(FLAGS.train_steps):
    ...
    if (i + 1) % FLAGS.validate_every == 0:
        ...
        '''
        test_loss, test_accuracy, test_summary = sess.run([nn.loss, nn.accuracy, nn.merged],
                                               feed_dict={nn.input: x_test,
                                                          nn.target: y_test,
                                                          nn.seq_len: test_seq_len,
                                                          nn.dropout_keep_prob: 1})
        #validation_writer.add_summary(summary, i)  # Write validation summary for step i (TensorBoard visualization)
        print('   test loss: {0:.4f} (accuracy {1:.4f})'.format(test_loss, test_accuracy))
        '''
# Save model
